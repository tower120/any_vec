...
fn remove_test() {
    ...
    // remove first
    vec.remove(0);
    assert_equal(vec.as_slice(), &[String::from("1"), String::from("3")]);
}
...
fn swap_remove_test() {
    ...
    vec.swap_remove(2);
    assert_equal(vec.as_slice(), &[String::from("0"), String::from("4")]);
}
...
fn any_vec_drain_in_the_middle_test() {
    ...
    assert_equal(vec2, [String::from("1"), String::from("2")]);
    assert_equal(
...
fn any_vec_splice_lifetime_test() {
    ...
    {
        let mut vec = any_vec.downcast_mut::<String>().unwrap();
        let replace = [String::from("100"), String::from("200")];
