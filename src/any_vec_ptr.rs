...
impl<Traits: ?Sized + Trait, M: MemBuilder> From<NonNull<AnyVec<Traits, M>>>
    for AnyVecPtr<Traits, M>
{
    #[inline]
...
pub(crate) mod utils {
    ...
    #[inline]
    pub unsafe fn element_ptr_at<AnyVecPtr: IAnyVecRawPtr>(
        ...
        if Unknown::is::<AnyVecPtr::Element>() {
            ...
            AnyVecTyped::<AnyVecPtr::Element, _>::new(NonNull::from(any_vec_raw))
                .as_ptr()
                .add(index) as *const _ as *const u8
        }
    }

    #[inline]
    pub unsafe fn element_mut_ptr_at<AnyVecPtr: IAnyVecRawPtr>(
    ...
    pub unsafe fn move_elements_at<AnyVecPtr: IAnyVecRawPtr>(
        ...
        if Unknown::is::<AnyVecPtr::Element>() {
            let any_vec_raw = any_vec_ptr.any_vec_raw();
            ptr::copy(src, dst, any_vec_raw.element_layout().size() * len);
        } else {
            ptr::copy(
