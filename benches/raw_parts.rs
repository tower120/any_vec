...
fn vec_from_raw_parts_push() {
    let mut vec = Vec::new();
    for _ in 0..SIZE {
        let mut vec = unsafe { Vec::from_raw_parts(vec.as_mut_ptr(), vec.len(), vec.capacity()) };
        vec.push(VALUE.clone());
        mem::forget(vec);
    }
}
...
pub fn bench_push(c: &mut Criterion) {
    ...
    c.bench_function("Vec from_raw_parts push", |b| {
        b.iter(|| vec_from_raw_parts_push())
    });
    c.bench_function("AnyVec push", |b| b.iter(|| any_vec_push()));
    c.bench_function("AnyVec from_raw_parts push", |b| {
        b.iter(|| any_vec_from_parts_push())
    });
}
