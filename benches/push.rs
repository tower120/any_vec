use any_vec::any_value::{AnyValueRaw, AnyValueSizelessRaw};
use any_vec::AnyVec;
...
fn any_vec_push_untyped_unchecked(size: usize) {
    ...
    for i in 0..size {
        let value = make_element(i);
        let raw_value = unsafe { AnyValueSizelessRaw::new(NonNull::from(&value).cast::<u8>()) };
        unsafe {
...
pub fn bench_push(c: &mut Criterion) {
    ...
    c.bench_function("AnyVec push untyped unchecked", |b| {
        b.iter(|| any_vec_push_untyped_unchecked(black_box(SIZE)))
    });
    c.bench_function("AnyVec push untyped", |b| {
        b.iter(|| any_vec_push_untyped(black_box(SIZE)))
    });
}
