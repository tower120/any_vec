...
pub fn bench_push(c: &mut Criterion) {
    ...
    c.bench_function("AnyVecTyped insert front", |b| {
        b.iter(|| any_vec_typed_insert_front())
    });
    ...
    c.bench_function("AnyVecTyped insert back", |b| {
        b.iter(|| any_vec_typed_insert_back())
    });
}
